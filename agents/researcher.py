...
class ResearchAgent:
    ...
    async def run_initial_research(self, research_state: dict):
        ...
        print_agent_output(
            f"Running initial research on the following query: {query}", agent="RESEARCHER")
    ...
    async def run_depth_research(self, draft_state: dict):
        ...
        print_agent_output(
            f"Running in depth research on the following report topic: {topic}", agent="RESEARCHER")
