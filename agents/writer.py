...
class WriterAgent:
    ...
    def write_sections(self, research_state: dict):
        ...
        response = call_model(prompt, task.get("model"),
                              max_retries=2, response_format='json')
...
"""
        ...
        response = call_model(prompt, task.get(
            "model"), response_format='json')
        return {"headers": json.loads(response)}

    def run(self, research_state: dict):
        print_agent_output(
            f"Writing final research report based on research data...", agent="WRITER")
        ...
        if research_state.get("task").get("follow_guidelines"):
            ...
            headers = self.revise_headers(task=research_state.get(
                "task"), headers=headers).get("headers")
